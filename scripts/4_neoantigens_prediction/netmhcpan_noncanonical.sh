...
#SBATCH -J NetMHCpan          # job name
#SBATCH -o /users/genomics/marta/logs/NetMHCpan_%A_%a.out    # File to which standard out will be written
#SBATCH -e /users/genomics/marta/logs/NetMHCpan_%A_%a.err    # File to which standard err will be written

PROJECT=$1
DIR=$2
CLUSTERDIR=$3/${PROJECT}/11_PeptideBindingMHC
mkdir $CLUSTERDIR
CLUSTERDIR=$CLUSTERDIR/noncanonical_CIPHER
mkdir $CLUSTERDIR
GENOMEDIR=$4
...
while IFS=, read patient normal tumor; do
    array=(${array[@]} $DIR/analysis/11_PeptideBindingMHC/noncanonical_CIPHER/${patient}/${patient}_noncanonical_all_orfs_FPKM1_notduplis_PROTEIN_9aa_cleanheader.fa) # Add new element at the end of the array
