...
#SBATCH -J NetMHCpan_te          # job name
#SBATCH -o /users/genomics/marta/logs/NetMHCpan_te_%A_%a.out    # File to which standard out will be written
#SBATCH -e /users/genomics/marta/logs/NetMHCpan_te_%A_%a.err    # File to which standard err will be written

PROJECT=$1
DIR=$2
CLUSTERDIR=$3/${PROJECT}/11_PeptideBindingMHC
mkdir $CLUSTERDIR
CLUSTERDIR=$CLUSTERDIR/translation_evidence_NOCDS
mkdir $CLUSTERDIR
...
while IFS=, read patient normal tumor; do
    array=(${array[@]} $DIR/analysis/11_PeptideBindingMHC/translation_evidence_NOCDS/${patient}/${patient}_translational_evidence_PROTEIN.fa) # Add new element at the end of the array
...
Rscript $DIR/scripts/4_neoantigens_prediction/netmhcpan/parse_netmhcpan_EL_BA_out_MODIF_${hla}.R $file

pid=$!
wait $pid


scp -rp ${CLUSTERDIR}/${patient} "$(whoami)"@"$(hostname -s)":$DIR/analysis/11_PeptideBindingMHC/translation_evidence_NOCDS
